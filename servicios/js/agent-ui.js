...
const AgentUI = (() => {
  ...
  function _agentCard(agent, matchResult) {
    const matchBadge = matchResult
      ? `<div class="agt-match-badge">${escapeHTML(matchResult.matchPercentage)}% Match</div>`
      : '';
    const initials = agent.name.split(' ').map(w => w[0]).join('').slice(0, 2);

    return `
      <div class="agt-card" onclick="AgentUI.showProfile('${escapeAttr(agent.id)}')">
        ${matchBadge}
        ${agent.verified ? '<div class="agt-verified-badge">✓ Verificado</div>' : ''}
        <div class="agt-avatar">${escapeHTML(initials)}</div>
        <div class="agt-card-name">${escapeHTML(agent.name)}</div>
        <div class="agt-card-rating">⭐ ${escapeHTML(agent.rating)} (${escapeHTML(agent.reviewCount)})</div>
        <div class="agt-card-zones">📍 ${escapeHTML(agent.zones.slice(0, 3).join(', '))}</div>
        <div class="agt-card-specs">
          ${agent.specialties.map(s => `<span class="agt-spec-tag">${escapeHTML(s)}</span>`).join('')}
        </div>
        <div class="agt-card-stats">
          <span>⏱️ ${escapeHTML(agent.responseTime)}</span>
          <span>📅 ${escapeHTML(agent.yearsExperience)}a exp.</span>
          <span>🏠 ${escapeHTML(agent.closedDeals)} ops.</span>
        </div>
        ${matchResult && matchResult.reasons.length > 0 ? `
          <div class="agt-match-reason">${escapeHTML(AgentMatch.getMatchExplanation(matchResult))}</div>
        ` : ''}
        <button class="svc-fin-cta" style="margin-top:auto" onclick="event.stopPropagation();AgentUI.showProfile('${escapeAttr(agent.id)}')">Ver perfil →</button>
      </div>`;
  }
      ...
      <div class="val-step">
        ...
        <div class="agt-operation-grid">
          ${Object.entries(OPERATION_LABELS).map(([val, label]) => `
            <button class="agt-operation-btn${_wizardData.operation === val ? ' active' : ''}" onclick="AgentUI.updateWizard('operation','${escapeAttr(val)}')">
        ...
        <div class="val-row">
          <div class="val-field val-full">
            ...
            <select onchange="AgentUI.updateWizard('district',this.value)">
              ...
              ${districts.map(d => `<option value="${escapeAttr(d)}"${_wizardData.district === d ? ' selected' : ''}>${escapeHTML(d)}</option>`).join('')}
            </select>
          </div>
        </div>
        <div class="val-row">
          <div class="val-field">
            ...
            <select onchange="AgentUI.updateWizard('propertyType',this.value)">
              ...
              ${types.map(t => `<option value="${escapeAttr(t.value)}"${_wizardData.propertyType === t.value ? ' selected' : ''}>${escapeHTML(t.label)}</option>`).join('')}
            </select>
          </div>
          <div class="val-field">
            <label>Área (m²)</label>
            <input type="number" min="10" placeholder="Ej: 120" value="${escapeAttr(_wizardData.area || '')}" oninput="AgentUI.updateWizard('area',+this.value)"/>
          </div>
        </div>
        <div class="val-row">
          <div class="val-field val-full">
            ...
            <select onchange="AgentUI.updateWizard('priceRange',this.value)">
              ...
              ${PRICE_RANGES.map(r => `<option value="${escapeAttr(r.value)}"${_wizardData.priceRange === r.value ? ' selected' : ''}>${escapeHTML(r.label)}</option>`).join('')}
            </select>
          </div>
        </div>
      </div>`;
      ...
      <div class="val-step">
        ...
        <div class="val-row">
          <div class="val-field val-full">
            ...
            <textarea rows="3" placeholder="Describe brevemente tu situación o lo que buscas..." oninput="AgentUI.updateWizard('notes',this.value)">${escapeHTML(_wizardData.notes || '')}</textarea>
          </div>
        </div>
      </div>`;
  }
      ...
      <div class="val-result-wrap">
        ...
        <div class="agt-results-header">
          ...
          <p style="text-align:center;color:var(--ink-3);font-size:13px;margin-bottom:20px">
            Para ${escapeHTML(opLabel.toLowerCase())} ${_wizardData.propertyType ? escapeHTML(PROPERTY_TYPES.find(t => t.value === _wizardData.propertyType)?.label || '') : ''} en ${escapeHTML(_wizardData.district || 'Lima')}
          </p>
        </div>
        ...
        ` : `
          <div style="text-align:center;padding:32px;color:var(--ink-3)">
            ...
            <button class="plan-cta outline" style="margin-top:12px" onclick="AgentUI.startWizard('${escapeAttr(_wizardData.operation)}')">Modificar búsqueda</button>
          </div>
        `}
      ...
      <div class="val-form-wrap">
        ...
        <div class="agt-profile-header">
          ...
          <div class="agt-profile-info">
            <h2 class="font-serif" style="font-size:22px;margin:0 0 4px">${escapeHTML(agent.name)}</h2>
            ${agent.verified ? '<div class="agt-verified-inline">✓ Agente verificado</div>' : ''}
            <div class="agt-profile-rating">⭐ ${escapeHTML(agent.rating)}/5 · ${escapeHTML(agent.reviewCount)} reseñas</div>
          </div>
          ${matchResult ? `<div class="agt-match-badge agt-match-lg">${escapeHTML(matchResult.matchPercentage)}% Match</div>` : ''}
        </div>

        ${matchResult && matchResult.reasons.length > 0 ? `
        <div class="agt-match-explain">
          <strong>¿Por qué este agente?</strong>
          <p>${escapeHTML(AgentMatch.getMatchExplanation(matchResult))}</p>
        </div>` : ''}
        ...
        <div class="agt-stats-grid">
          <div class="agt-stat-card">
            <div class="agt-stat-value">${escapeHTML(agent.yearsExperience)}</div>
            <div class="agt-stat-label">Años exp.</div>
          </div>
          <div class="agt-stat-card">
            <div class="agt-stat-value">${escapeHTML(agent.closedDeals)}</div>
            <div class="agt-stat-label">Operaciones</div>
          </div>
          <div class="agt-stat-card">
            <div class="agt-stat-value">${escapeHTML(agent.avgDaysToClose)}d</div>
            <div class="agt-stat-label">Tiempo prom.</div>
          </div>
          <div class="agt-stat-card">
            <div class="agt-stat-value">${escapeHTML(agent.activeListings)}</div>
        ...
        <div class="leg-detail-section">
          ...
          <div class="agt-zone-tags">
            ${agent.zones.map(z => `<span class="agt-spec-tag">${escapeHTML(z)}</span>`).join('')}
          </div>
        </div>
        ...
        <div class="leg-detail-section">
          ...
          <div class="agt-zone-tags">
            ${agent.propertyTypes.map(t => {
              ...
              return `<span class="agt-spec-tag">${escapeHTML(pt ? pt.label : t)}</span>`;
            }).join('')}
          </div>
        </div>
  ...
  function _renderContactFlow() {
    ...
    return `
      <div class="val-form-wrap">
        <button class="val-back" onclick="AgentUI.showProfile('${escapeAttr(_selectedAgentId)}')">← Volver al perfil</button>

        <div class="val-form-header">
          <div class="val-form-badge">Solicitud para ${escapeHTML(agent.name)}</div>
        ...
        <div class="val-row">
          <div class="val-field val-full">
            ...
            <select onchange="AgentUI.updateContact('operation',this.value)">
              ...
              ${Object.entries(OPERATION_LABELS).map(([v, l]) => `<option value="${escapeAttr(v)}"${_contactData.operation === v ? ' selected' : ''}>${escapeHTML(l)}</option>`).join('')}
            </select>
          </div>
        </div>
        <div class="val-row">
          <div class="val-field">
            ...
            <select onchange="AgentUI.updateContact('district',this.value)">
              <option value="">Selecciona distrito</option>
              ${districts.map(d => `<option value="${escapeAttr(d)}"${_contactData.district === d ? ' selected' : ''}>${escapeHTML(d)}</option>`).join('')}
            </select>
          </div>
          <div class="val-field">
            <label>Tipo de propiedad</label>
            <select onchange="AgentUI.updateContact('propertyType',this.value)">
              <option value="">Selecciona tipo</option>
              ${types.map(t => `<option value="${escapeAttr(t.value)}"${_contactData.propertyType === t.value ? ' selected' : ''}>${escapeHTML(t.label)}</option>`).join('')}
            </select>
          </div>
        </div>
        <div class="val-row">
          <div class="val-field">
            <label>Área (m²)</label>
            <input type="number" min="10" placeholder="Ej: 120" value="${escapeAttr(_contactData.area || '')}" oninput="AgentUI.updateContact('area',+this.value)"/>
          </div>
          <div class="val-field">
            <label>Rango de precio</label>
            <select onchange="AgentUI.updateContact('priceRange',this.value)">
              <option value="">Selecciona</option>
              ${PRICE_RANGES.map(r => `<option value="${escapeAttr(r.value)}"${_contactData.priceRange === r.value ? ' selected' : ''}>${escapeHTML(r.label)}</option>`).join('')}
            </select>
          </div>
        </div>
        <div class="val-row">
          <div class="val-field val-full">
            ...
            <input type="text" placeholder="Av. ejemplo 1234" value="${escapeAttr(_contactData.address || '')}" oninput="AgentUI.updateContact('address',this.value)"/>
          </div>
        </div>
      </div>`;
  }
      ...
      <div class="val-step">
        ...
        <div class="val-row">
          <div class="val-field val-full">
            ...
            <textarea rows="3" placeholder="Cuéntale al agente sobre tu situación..." oninput="AgentUI.updateContact('message',this.value)">${escapeHTML(_contactData.message || '')}</textarea>
          </div>
        </div>
      </div>`;
  }
    ...
    return `
      <div class="val-step" style="text-align:center;padding:24px 0">
        ...
        <p style="color:var(--ink-3);font-size:14px;margin-bottom:20px;max-width:420px;margin-left:auto;margin-right:auto">
          Tu solicitud ha sido enviada a <strong>${escapeHTML(agent.name)}</strong>.
          El agente te contactará en un máximo de <strong>${escapeHTML(agent.responseTime)}</strong>.
        </p>
  ...
  function contactNext() {
    if (_contactStep === 1) {
      const container = document.querySelector('.val-step');
      if (container) RecoValidation.clearAllErrors(container);

      const nameResult = RecoValidation.validateName(_contactData.contactName || '');
      const emailResult = RecoValidation.validateEmail(_contactData.contactEmail || '');
      const phoneResult = RecoValidation.validatePhone(_contactData.contactPhone || '');
      const msgResult = RecoValidation.validateText(_contactData.message || '', { required: false, maxLength: 1000 });

      let hasError = false;

      const nameInput = document.querySelector('input[type="text"][placeholder="Tu nombre"]');
      const phoneInput = document.querySelector('input[type="tel"]');
      const emailInput = document.querySelector('input[type="email"]');
      const msgInput = document.querySelector('.val-step textarea');

      if (!nameResult.valid) {
        if (nameInput) RecoValidation.showFieldError(nameInput, nameResult.error);
        hasError = true;
      }
      if (!emailResult.valid) {
        if (emailInput) RecoValidation.showFieldError(emailInput, emailResult.error);
        hasError = true;
      }
      if (!phoneResult.valid) {
        if (phoneInput) RecoValidation.showFieldError(phoneInput, phoneResult.error);
        hasError = true;
      }
      if (!msgResult.valid) {
        if (msgInput) RecoValidation.showFieldError(msgInput, msgResult.error);
        hasError = true;
      }
