...
**Generated:** 2026-09-09T04:44:04.956Z
**Phase:** 2B-0.5
**Analysis date:** 2026-09-09T04:43:58.615Z
...
**Dataset Utility:**
- block_candidate: 399 features → block_master (conceptual, not in current architecture)
  - ⚠ Includes 140 small polygons reclassified from area-only PARCEL to BLOCK (dataset is manzana-level)
...
**Performance:** read=0ms, decompress=0ms, parse=1ms, total=1ms
...
**Recommendation:** NOT FOR PARCEL_MASTER — Data is BLOCK. Useful as: block_candidate(399). Need parcel-level source for this district.
...
`coverage_pct = (sum of polygon areas inside district boundary) / (district total area) × 100`

This is **territorial area coverage**, not feature count ratio. It measures what fraction of
the district's land area is covered by dataset polygons. Limitations: overlapping polygons
can inflate coverage; gaps between parcels (streets, parks) reduce it. A district fully
covered by parcel data will not reach 100% because streets and public spaces have no parcels.

### PARCEL_LEVEL_USABLE Definition

A dataset is considered PARCEL_LEVEL_USABLE only if ALL of:

1. Represents individual predios (parcels), not manzanas (blocks) or zones
2. Geometry is valid (Hard Gate B passes)
3. Semantic classification has sufficient confidence (area distribution consistent with parcels)
4. Coverage is measurable (Hard Gate C evaluated)
5. Provenance is traceable (source identified, acquisition documented)
6. Does not contradict higher-authority sources
7. Does not degrade existing parcel_master data
...
- **SURQUILLO:** Data classified as BLOCK — NOT usable for parcel_master. Potentially useful as: block_candidate(399). Need parcel-level source.
...
---
*Generated by tools/etl/pilot_report.js — 2026-09-09T04:44:04.990Z*